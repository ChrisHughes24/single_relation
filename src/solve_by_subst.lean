...
/-- Auxiliary definition used for `check_subst`.
  Assuming `l = check_subst T r₂`, `r₁.reverse ++ r₂`is reduced, then it will
  return `check_subst T (of_list (r₁.reverse ++ r₂))`     -/
@[inline] meta def check_subst_core : Π (r₁ r₂ : list (Σ i : ι, C∞))
  (l : list (ι × bool × (free_group ι × free_group ι))),
  list (ι × bool × (free_group ι × free_group ι))
| []          w₂ l := l
| (⟨i,a⟩::w₁) w₂ l := check_subst_core w₁ (⟨i, a⟩::w₂) $
  if i ∈ T
    then l
    else if a = of_add 1
      then if l.any (λ j, j.1 = i)
        then l
        else (i, tt, of_list w₁.reverse, of_list w₂) :: l
      else if a = of_add (-1)
        then if l.any (λ j, j.1 = i)
          then l
          else (i, ff, of_list w₁.reverse, of_list w₂) :: l
        else l.erasep (λ j, j.1 = i)
...
@[inline] meta def check_subst (r : free_group ι) :
  list (ι × bool × free_group ι × free_group ι) :=
check_subst_core T r.to_list.reverse [] []
...
| []            := none
| (⟨i, tt, w₁, w₂⟩ :: l) := some $ (⟨of (w₁⁻¹), w₁⁻¹ * w₂⁻¹⟩, i)
| (⟨i, ff, w₁, w₂⟩ :: l) := some $ (⟨(of w₂)⁻¹, w₂ * w₁⟩, i)
end
